...
// BEGIN: ESP-IDF PSRAM compatibility shim (replace Arduino.h for PSRAM helpers)
#pragma once
...
#ifndef PSRAM_LOG_TAG
#define PSRAM_LOG_TAG "psram_compat"
#endif

// Return true if PSRAM (external SPIRAM) is available and initialized.
static inline bool psramFound() noexcept {
    // esp_spiram_is_initialized() is safe to call even if SPIRAM support is not compiled in.
    // If the symbol is unavailable for your ESP-IDF version, uncomment the fallback (below).
#ifdef CONFIG_SPIRAM_SUPPORT
    return esp_spiram_is_initialized() != 0;
#else
    // Fallback check: look for any allocatable PSRAM block (safer fallback if CONFIG_SPIRAM_SUPPORT not defined)
    // Note: heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM) is also valid, but may not exist on all SDK versions.
    return esp_spiram_is_initialized() != 0;
#endif
}

// Allocate memory preferentially in PSRAM when available, otherwise fall back to normal heap.
static inline void* ps_malloc(size_t size) {
    if (size == 0) return nullptr;
    if (psramFound()) {
        void* p = heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (p) return p;
        // If PSRAM allocation fails, try normal heap as fallback
    }
    return malloc(size);
}

// Reallocate memory: prefer PSRAM-backed reallocation if PSRAM is initialized.
// Note: heap_caps_realloc signature differs by SDK; heap_caps_realloc exists in modern esp-idf.
static inline void* ps_realloc(void* ptr, size_t size) {
    if (ptr == nullptr) {
        return ps_malloc(size);
    }
    if (size == 0) {
        // Free and return nullptr
        if (psramFound()) heap_caps_free(ptr); else free(ptr);
        return nullptr;
    }
    if (psramFound()) {
        // try PSRAM reallocation first
        void* p = heap_caps_realloc(ptr, size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (p) return p;
        // if it fails, fall through and try normal realloc (may move to internal heap)
    }
    return realloc(ptr, size);
}

// Free memory allocated via ps_malloc/ps_realloc; will free from correct allocator.
static inline void ps_free(void* ptr) noexcept {
    if (!ptr) return;
    // When memory might come from PSRAM or normal heap, best attempt PSRAM free:
    // heap_caps_free works for memory allocated by heap_caps_malloc/realloc.
    // If heap_caps_free does not accept a pointer from normal heap, fall back to free().
    // But in practice heap_caps_free handles PSRAM allocations; use free() if unsure.
    if (psramFound()) {
        // attempt PSRAM free first (safe for PSRAM pointers)
        heap_caps_free(ptr);
    } else {
        free(ptr);
    }
...
// Duplicate string into PSRAM-aware allocation
...
// END: ESP-IDF PSRAM compatibility shim
