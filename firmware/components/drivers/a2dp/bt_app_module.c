...
#define CUSTOM_AR_TAG           "AR: >>>>>>>>>>>>>"


// The defined MAC address is only used for testing
# define BT_SINK                 "Itel ITW-60"
# define BT_SINK_MAC             {0x20, 0x08, 0x14, 0xA1, 0x0C, 0x23} 
...
// device mac address 
//const char* device_name = BT_SINK; 
//const uint8_t device_mac_addr[6] = BT_SINK_MAC; 
...
void stream_a2dp_to_device(void)
{
    // set log to warning
    esp_log_level_set("*", ESP_LOG_WARN);

    // Initialize NVS.
    // May conflict with wifi subsystem nvs_flash_init(). 
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir) {
        ...
        ESP_LOGI(CUSTOM_AR_TAG, "Checking potential device, address %s, name %s", bda_str, s_peer_bdname);
    ...
    switch (event) {
        ...
        case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: {
            if (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STOPPED) {
                ...
                if (already_paired) {
                    ESP_LOGI(CUSTOM_AR_TAG, "Forcing a connection...");
                    uint8_t sink_addr[6] = BT_SINK_MAC; 

                    memcpy(s_peer_bda, sink_addr, sizeof(sink_addr));
                    memcpy(s_peer_bdname, BT_SINK, sizeof(BT_SINK));
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    switch (event) {
        case BT_APP_EVT_STACK_UP: {
            ...
            char bda_str[18];
            ESP_LOGI(CUSTOM_AR_TAG, "Current ESP32 BT MAC: %s", bda2str(esp_bt_dev_get_address(), bda_str, 18));

            // go through all paired devices in security database
            if(ESP_OK == tError) {
                for(int i = 0; i < paired_devs; i++) {
                    ESP_LOGI(CUSTOM_AR_TAG, "Pair device database entry: %s", bda2str(paired_devs_macs[i], bda_str, 18));
                    uint8_t sink_addr[6] = BT_SINK_MAC; 
                    if (compare_arrays(paired_devs_macs[i], sink_addr , 6)) {
                        ESP_LOGI(CUSTOM_AR_TAG, "Device already paired.");
            ...
            do {
                ...
                s_tmr = xTimerCreate("connTmr", (BT_EVENT_TIMER / portTICK_PERIOD_MS),
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
        return flac_a2d_data_cb(data, len);
}
        ...
        case APP_AV_MEDIA_STATE_STARTING: {
            if (event == ESP_A2D_MEDIA_CTRL_ACK_EVT) {
                ...
                if (a2d->media_ctrl_stat.cmd == ESP_A2D_MEDIA_CTRL_START &&
                    ...
                    decoder_start("/sdcard/MUSIC/15. Radiohead - Lift.flac");
                } else {
